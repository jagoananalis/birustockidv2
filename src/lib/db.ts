...
function getRuntimeDbConfig(): { source: DbSource; databaseUrl?: string } {
  ...
  // Customer must never silently fall back to local PGLite on Vercel.
...
async function createSql(): Promise<Sql> {
  ...
  return config.source === "neon"
    ? createNeonSql(config.databaseUrl as string)
    : createPgliteSql();
}

/**
 * Get the shared, **server-only** SQL client. Neon when `DATABASE_URL` is set. On Vercel, missing `DATABASE_URL` throws.
 * Local development may use the PGLite fallback. Memoized — safe to call per request.
 *
...
/**
 * The shared PGLite instance (local development only), with `migrations/*.sql` applied.
