        ...
        <div className="card-body">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="text-[12px] text-subtle">
              {item.timeframe} · {formatIdDate(item.publishedAt)}
            </span>
          </div>
          <div>
            <div className="mb-2 flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-wide text-subtle">
              <span>{item.bias}</span>
              {item.target ? <span>· Target {item.target}</span> : null}
            </div>
            <h3 className="text-[17px] leading-snug font-bold text-ink transition-colors group-hover:text-primary">
              {item.title}
            </h3>
            <p className="mt-1 line-clamp-3 text-sm text-muted">{item.excerpt}</p>
          </div>
