...
import { deleteAnalisis, deleteEdukasi, founderLogin, founderPing, listAnalisisAdmin, listEdukasiAdmin, saveAnalisis, saveEdukasi, type Accent, type AnalisisItem, type ContentStatus, type EdukasiItem, type EduLevel } from "@/lib/content";
import { deleteNews, listNewsAdmin, saveNews, type NewsItem, type NewsStatus, type NewsThumb } from "@/lib/news";
...
function StudioPage() {
  ...
  const refresh = useCallback(async () => {
    if (!token) return;
    const [a, n, e] = await Promise.all([listAnalisisAdmin({ data: { token } }), listNewsAdmin({ data: { token } }), listEdukasiAdmin({ data: { token } })]);
...
function AnalisisStudio({ token, items, onChange, busy, setBusy }: { token: string; items: AnalisisItem[]; onChange: () => Promise<void>; busy: boolean; setBusy: (v: boolean) => void }) {
  const empty = useMemo(() => ({ id: undefined as number | undefined, pair: "XAU/USD", title: "", excerpt: "", body: "", imageUrl: "", accent: "blue" as Accent, publishedAt: new Date().toISOString().slice(0, 10), status: "DRAFT" as ContentStatus, timeframe: "H4", bias: "Bullish", support: "", resistance: "", target: "", invalidation: "", scenarioBullish: "", scenarioBearish: "" }), []);
...
function NewsStudio({ token, items, onChange, busy, setBusy }: { token: string; items: NewsItem[]; onChange: () => Promise<void>; busy: boolean; setBusy: (v: boolean) => void }) {
  const empty = useMemo(() => ({ id: undefined as number | undefined, category: "Ekonomi Global", title: "", excerpt: "", body: "", thumb: "capitol" as NewsThumb, publishedAt: new Date().toISOString().slice(0, 10), status: "DRAFT" as NewsStatus }), []); const [form, setForm] = useState(empty); const [error, setError] = useState("");
...
function EdukasiStudio({ token, items, onChange, busy, setBusy }: { token: string; items: EdukasiItem[]; onChange: () => Promise<void>; busy: boolean; setBusy: (v: boolean) => void }) {
  const empty = useMemo(() => ({ id: undefined as number | undefined, level: "Pemula" as EduLevel, title: "", description: "", body: "", imageUrl: "", publishedAt: new Date().toISOString().slice(0, 10), status: "DRAFT" as ContentStatus }), []); const [form, setForm] = useState(empty); const [error, setError] = useState("");
